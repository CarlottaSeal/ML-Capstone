...
EXPERIMENT_NAME="WTI_trading_update${TIMESTAMP}"
...
if [ -d "./results" ]; then
    for dir in ./results/long_term_forecast_*; do
        if [ -d "$dir" ] && [ "$dir" -nt "${START_TIMESTAMP}" ]; then
...
for f in ./result_long_term_forecast_*.txt; do
    if [ -f "$f" ] && [ "$f" -nt "${START_TIMESTAMP}" ]; then
        cp "$f" ${RESULTS_DIR}/
